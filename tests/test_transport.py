"""Tests for aionanit_jr.ws.transport — WsTransport."""
...
from aionanit_jr.exceptions import NanitConnectionError, NanitTransportError
from aionanit_jr.models import ConnectionState, TransportKind
from aionanit_jr.ws.transport import WsTransport
...
def _make_transport(
    ...
    # spec= leaves .closed a truthy Mock; the reconnect loop keys on it.
...
class TestGetHeadersCallback:
    ...
    async def test_reconnect_calls_get_headers(self) -> None:
        ...
        with patch("aionanit_jr.ws.transport.asyncio.sleep", new_callable=AsyncMock):
...
class TestReconnectOnClosedSession:
    """The aiohttp session belongs to the caller. Once it is closed every
    ws_connect raises RuntimeError("Session is closed") forever, so the
    reconnect loop must stop rather than treat it as a transient blip."""

    @staticmethod
    def _prime(t: WsTransport) -> None:
        # State as if previously connected and the recv loop ended.
        t._url = "wss://api.nanit.com/focus/cameras/cam1/user_connect"
        t._headers = {"Authorization": "Bearer tok"}
        t._transport_kind = TransportKind.CLOUD
        t._closed = False

    async def test_stops_when_session_is_closed(self, caplog) -> None:
        t, session, _, conn_cb = _make_transport()
        self._prime(t)
        session.closed = True
        session.ws_connect = AsyncMock(side_effect=RuntimeError("Session is closed"))

        # Drive through _ensure_reconnect_task so the loop is the registered
        # owner; a direct await bails before reaching the code under test.
        # A short real sleep (not a no-op mock) keeps a retrying loop yielding
        # so wait_for can time it out instead of spinning forever.
        # The patch target IS the global asyncio.sleep, so hold the real one.
        real_sleep = asyncio.sleep

        async def _short_sleep(*_a, **_k) -> None:
            await real_sleep(0.01)

        with patch("aionanit_jr.ws.transport.asyncio.sleep", _short_sleep):
            t._ensure_reconnect_task()
            assert t._reconnect_task is not None
            try:
                await asyncio.wait_for(t._reconnect_task, timeout=0.5)
            except asyncio.TimeoutError:
                pytest.fail(
                    "reconnect loop kept retrying on a closed session; "
                    f"warnings: {[r.getMessage() for r in caplog.records if r.levelname == 'WARNING']}"
                )

        assert t._closed is True
        assert session.ws_connect.await_count == 1
        conn_cb.assert_called_with(ConnectionState.DISCONNECTED, TransportKind.CLOUD, None)
        assert not any("Reconnect failed" in r.getMessage() for r in caplog.records)

    async def test_transient_client_error_still_retries(self) -> None:
        t, session, _, conn_cb = _make_transport()
        self._prime(t)
        ...
        async def _block(self_=None):
            await gate.wait()
            yield  # pragma: no cover - never reached

        mock_ws = AsyncMock(spec=aiohttp.ClientWebSocketResponse)
        mock_ws.closed = False
        mock_ws.__aiter__ = _block
        session.ws_connect = AsyncMock(side_effect=[aiohttp.ClientError("blip"), mock_ws])

        # Shrink the backoff rather than mock sleep: a no-op sleep makes the
        # keepalive loop started on reconnect spin without ever yielding.
        with (
            patch("aionanit_jr.ws.transport._INITIAL_BACKOFF", 0.001),
            patch("aionanit_jr.ws.transport._JITTER_MAX", 0.0),
        ):
            t._ensure_reconnect_task()
            assert t._reconnect_task is not None
            await asyncio.wait_for(t._reconnect_task, timeout=0.5)
        ...
        conn_cb.assert_called_with(ConnectionState.CONNECTED, TransportKind.CLOUD, None)
