...
from aionanit_jr.camera import (
...
from aionanit_jr.models import TransportKind
...
async def test_token_refresh_forces_reconnect_before_expiry() -> None:
    ...
    with patch("aionanit_jr.camera.asyncio.sleep", AsyncMock(return_value=None)):
...
async def test_token_refresh_skips_reconnect_when_token_already_fresh() -> None:
    ...
    with patch("aionanit_jr.camera.asyncio.sleep", AsyncMock(side_effect=_sleep_then_stop)):
        await asyncio.wait_for(camera._token_refresh_loop(), timeout=0.1)
