...
@dataclass
class DiaryData:
    """Runtime data for one baby's care log (diary + sleep feed)."""

    baby: Baby
    coordinator: NanitDiaryCoordinator
    rolling: NanitRollingAverageCoordinator | None = None
...
class NanitHub:
    ...
    def __init__(
        ...
        self._diary_data: dict[str, DiaryData] = {}
    ...
    @property
    def diary_data(self) -> dict[str, DiaryData]:
        """Return per-baby diary/sleep runtime data, keyed by baby_uid."""
        return self._diary_data
    ...
    async def async_setup(self) -> None:
        ...
        # Care log/sleep feed is baby-scoped, not device-scoped: set it up for
        # every baby regardless of whether its camera or speaker connected, so
        # a camera outage never takes the diary down with it. Non-fatal per
        # baby — same degrade-gracefully pattern as the cloud/network
        # coordinators above.
        for baby in babies:
            try:
                diary_coordinator = NanitDiaryCoordinator(self._hass, self._entry, self, baby)
                await diary_coordinator.async_config_entry_first_refresh()
            except ConfigEntryAuthFailed:
                raise
            except ConfigEntryNotReady as err:
                _LOGGER.warning(
                    "Diary coordinator for %s failed to start; care-log entities disabled (%s)",
                    baby.name,
                    err,
                )
                continue

            rolling_coordinator: NanitRollingAverageCoordinator | None = None
            try:
                rolling_coordinator = NanitRollingAverageCoordinator(self._hass, self._entry, self, baby)
                await rolling_coordinator.async_config_entry_first_refresh()
            except ConfigEntryAuthFailed:
                raise
            except ConfigEntryNotReady as err:
                _LOGGER.warning(
                    "Rolling-average coordinator for %s failed to start; "
                    "30-day average entities disabled (%s)",
                    baby.name,
                    err,
                )
                rolling_coordinator = None

            self._diary_data[baby.uid] = DiaryData(
                baby=baby, coordinator=diary_coordinator, rolling=rolling_coordinator
            )
