    ...
    async def _reconnect_loop(self) -> None:
        ...
        while not self._closed:
            ...
            try:
                ...
                # The session belongs to the caller. Once it is closed every
                # ws_connect raises RuntimeError("Session is closed") for the
                # life of the process, so retrying is not a blip to back off
                # from - it is a loop that never ends. Key on the session's own
                # state, never on the exception text.
                if self._session.closed:
                    _LOGGER.debug("Session closed by owner; stopping reconnect: %s", err)
                    self._closed = True
                    self._on_connection_change(
                        ConnectionState.DISCONNECTED, self._transport_kind, None
                    )
                    return
