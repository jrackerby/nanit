...
from .entity import NanitCloudEntity, NanitDiaryEntity, NanitEntity, NanitSoundLightEntity
...
async def async_setup_entry(
    ...
    for diary_data in entry.runtime_data.diaries.values():
        entities.append(NanitAsleepSensor(diary_data.coordinator))
...
class NanitAsleepSensor(NanitDiaryEntity, BinarySensorEntity):
    """Whether the baby is currently asleep, per the latest camera-detected transition.

    Derived from /feed's kind="event" rows (FELL_ASLEEP/PUT_IN_BED = asleep,
    WOKE_UP/REMOVED/LEAVING = awake) — see diary_api.py for provenance. Unknown
    (None) until the first transition inside the polled lookback window,
    which is the honest answer for a baby with no sleep event that recently.
    """

    _attr_device_class = BinarySensorDeviceClass.OCCUPANCY
    _attr_translation_key = "asleep"

    def __init__(self, coordinator: NanitDiaryCoordinator) -> None:
        """Initialize."""
        super().__init__(coordinator)
        self._attr_unique_id = f"{coordinator.baby.uid}_diary_asleep"

    @property
    def is_on(self) -> bool | None:
        """Return True when asleep, False when awake, None when unknown."""
        if self.coordinator.data is None:
            return None
        return self.coordinator.data.is_asleep()
